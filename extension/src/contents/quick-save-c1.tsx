...
export default function QuickSaveC1() {
	const [open, setOpen] = useState(false)

	useEffect(() => {
		const handler = (e: MessageEvent) => {
			if (e.data?.type === "pm:quick-save:open") setOpen(true)
			if (e.data?.type === "pm:quick-save:close") setOpen(false)
		}
		window.addEventListener("message", handler)

		const onRuntimeMessage = (msg: any) => {
			if (msg?.type === "pm:quick-save:open") setOpen(true)
			if (msg?.type === "pm:quick-save:close") setOpen(false)
		}
		try {
